...
|---|---|---|
| Active deployment | `v31_v8b_fusion_quickqual_v1` | `configs/base.yaml` + `artifacts/checkpoints/best.pt` |
...
| Meta-classifier | numeric StandardScaler + LogisticRegression parameters embedded in the composite checkpoint | `artifacts/checkpoints/best.pt` |
| Deployment threshold | `0.06` | `artifacts/evaluations/external_test_v31_v8b_fusion_quickqual_v1_best_metrics.json` |
| DDR holdout performance | AUROC 0.9341, Sens 0.8234, Spec 0.9086 | `artifacts/evaluations/external_test_v31_v8b_fusion_quickqual_v1_best_metrics.json` |
| Source fusion metric artifact | `classification_domains:late_fusion` | `artifacts/runs/99_misc/v31_v8b_late_fusion_quickqual_v1/evaluations/v31_v8b_late_fusion_quickqual_v1_metrics.json` |
| Active checkpoint metadata | `architecture=v31_v8b_fusion`, `optimal_threshold=0.06`, `decision_threshold=0.06` | `artifacts/checkpoints/best.pt` |
...
| Previous active rollback | `v31_v8b_fusion_features_hflip_v2` | `artifacts/checkpoints/best_pre_quickqual_v1_20260529.pt.bak` |
...
| `v31_v8b_late_fusion_quickqual_v1` | `99_misc` | `deployment_candidates` | `v31_v8b_late_fusion_quickqual_v1.yaml` | no checkpoint | external_holdout | QuickQual-line v8b lesion evidence | current active fusion meta source. `classification_domains:late_fusion` holdout AUROC 0.9341, threshold 0.06, Sens 0.8234, Spec 0.9086 |
...
| `v31_v8b_late_fusion_quickqual_v1.yaml` | QuickQual-line v31+v8b late-fusion meta source config | active fusion meta source under `99_misc`; holdout AUROC 0.9341, threshold 0.06 |
...
**Mask-geometry caveat**: Earlier pixel-mask-supervised conclusions for decoder/seg_head/standalone segmentation runs are confounded unless retrained after the geometry fix. A second, MAPLES-specific caveat was found on 2026-05-21: MAPLES annotation PNGs are ROI-space masks and must be restored through `MESSIDOR-ROIs.csv`. Therefore MAPLES metrics for pre-ROI-fix segmentation runs (`seg_evidence_v3` through original `v8`) are diagnostic only. The v31 base classifier path is not affected because it is image-only classification + CAM. The current active `v31_v8b_fusion_quickqual_v1` deployment uses the QuickQual-line `seg_evidence_v8b_quickqual_v1` evidence module; the previous hflip/circular active used the post-fix `seg_evidence_v8b_ddrseg_tjdr_maplesfix` evidence module.
...
- **`v31_v8b_fusion_quickqual_v1` is the active deployment alias** in `configs/base.yaml`, and `artifacts/checkpoints/best.pt` currently contains the composite QuickQual-line v31 classifier + QuickQual-line v8b segmenter + numeric meta-classifier checkpoint. Formal DDR 20% calibration / 80% holdout metrics: AUROC **0.9341**, threshold **0.06**, Sensitivity **0.8234**, Specificity **0.9086**. This is below the previous hflip/circular peak AUROC 0.9431, but it was promoted for train-serve preprocessing consistency.
