...
| `10_grounded_classifier` | Shortcut-free classifier redesign diagnostics and future grounded architectures | `v31_dfr_v1`, `bagnet_v1_p33_r256`, `bagnet_v1_p65_r512`, `cbm_v1_stage1`, `cbm_v1`, `v8b_evidence_classifier_v1`, `v8b_evidence_classifier_clsdomains_v1`, `v8b_evidence_classifier_aptos_v1`, `v8b_evidence_classifier_grid_v1`, `v31_v8b_late_fusion_sweep_v1`, `v31_v8b_fusion_v2` |
| `06_deployment_candidates` | Runs relevant to deployment decisions | `v17_focal_g2`, `v17_512_focal_g2`, `v21_512_layercam`, `v24_multitask`, `v28_no_attention`, `v30_gated_pooling`, `v31_no_se_gated`, `v31_v8b_fusion_v2` |
...
| `v31_no_se_gated` | `07_lesion_evidence` | `06_deployment_candidates` | `v31_no_se_gated.yaml` | yes | external | train/test XAI block4, shortcut audit | previous active and current fusion base classifier; val AUROC 0.9993, DDR AUROC 0.9160; D5-D7 shortcut audit supports domain/style shortcut reliance |
...
| `v31_v8b_fusion_v2` | `10_grounded_classifier` | `06_deployment_candidates` | `v31_v8b_fusion_v2.yaml` | yes | external_holdout, latency probe, service smoke | v8b lesion segmentation evidence | active deployment alias. Packages v31 classifier + v8b segmenter + numeric StandardScaler/LogReg meta-classifier into `artifacts/checkpoints/best.pt`; threshold 0.38; backend/frontend code unchanged |
...
| `v31_v8b_fusion_v2.yaml` | Active v31 + v8b score-level fusion runtime config | active deployment config; uses fixed checkpoint alias policy through `artifacts/checkpoints/best.pt` in `base.yaml` |
...
- **`v31_v8b_fusion_v2` is the active deployment alias** in `configs/base.yaml`, and `artifacts/checkpoints/best.pt` currently contains the composite v31 classifier + v8b segmenter + numeric meta-classifier checkpoint. Formal DDR 20% calibration / 80% holdout metrics: AUROC **0.9403**, threshold **0.38**, Sensitivity **0.8118**, Specificity **0.9238**. `v31_no_se_gated` is preserved as the base classifier source with DDR AUROC 0.9160 and test-split block4 Layer-CAM IoU top-20 0.0785.
...
- **Phase-0 gate 전 모델 FAIL**: test split 기준 center_gaussian+2σ threshold=0.1089. 모든 모델의 AUC-IoU(현재 최고 v37b/v39 test 0.0625)가 임계값 미달. 2σ gate 기준 재조정 필요 (test 기준 후보: +1σ=0.0728 또는 절대값 0.05).
...
- **Sprint 3 XAI 실험 완료**: v31~v35 + MAPLES-DR clean-cohort + OD masking. XAI 개선을 위해서는 도메인 불변 feature 학습 등 아키텍처 수준 접근 필요. 당시 배포는 v31로 유지했다.
