...
- **Model**: Active deployment is `v31_v8b_fusion_quickqual_v2` (promoted 2026-06-03; **v31 collinearity refit** of the QuickQual line). Same components as v1 — base classifier `v31_no_se_gated_quickqual_v1`, segmenter `seg_evidence_v8b_quickqual_v1` — but the numeric StandardScaler+LogReg meta-classifier now uses a **single v31 log-odds feature (`v31_logit`)** plus the v8b lesion features (**88 features total**; the redundant `v31_probability` column was dropped because `v31_probability=sigmoid(v31_logit)` is near-collinear and inflated/destabilized the coefficients). Deployment threshold **0.0856** is selected on the DDR 20% calibration split to match the previous active sensitivity. Formal DDR 80% holdout metrics: **AUROC 0.9360, threshold 0.0856, sensitivity 0.8316, specificity 0.9070** (`classification_domains:late_fusion:v31_logit`). At this matched-sensitivity operating point v2 weakly dominates the previous v1 (AUROC +0.0019, sensitivity +0.008, specificity −0.0016, accuracy +0.0033, F1 +0.0041). Phase 4-E shortcut audit still applies to the v31 base path, so product wording should describe v8b output as separately detected lesion candidate areas rather than causal proof.
