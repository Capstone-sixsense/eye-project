...
- **Model**: Active deployment is `v31_v8b_fusion_quickqual_v1` (promoted 2026-05-29 for **train-serve preprocessing consistency**, not for peak metric). v31_no_se_gated (retrained on QuickQual geometry) is the base classifier score source, v8b (retrained on QuickQual geometry) provides lesion segmentation evidence, and a numeric StandardScaler+LogReg meta-classifier produces the final abnormal probability. Formal DDR 20% calibration / 80% holdout metrics: AUROC 0.9341, threshold 0.06, sensitivity ~0.82, specificity ~0.91 (`classification_domains:late_fusion`, no hflip TTA). **This is −0.009 AUROC below the previous `v31_v8b_fusion_features_hflip_v2` (0.9431); the trade was accepted to make training geometry match the serve pipeline** (backend QuickQual square-pad → AI passthrough), and to preserve the full retinal field that circular crop clipped. The `§9 resize-skew` analysis showed classifier AUROC is preprocessing-skew-neutral, so the consistency gain carries low downside. Phase 4-E shortcut audit still applies to the v31 base path, so product wording should describe v8b output as separately detected lesion candidate areas rather than causal proof.
...
- **Previous active / rollback**: `v31_v8b_fusion_features_hflip_v2` (circular geometry, AUROC 0.9431) is the rollback baseline, backed up at `artifacts/checkpoints/best_pre_quickqual_v1_20260529.pt.bak`. `v31_v8b_fusion_v2` (circular) is the older baseline at `best_pre_features_hflip_v2_20260527.pt.bak`. To roll back, copy the `.bak` into `artifacts/checkpoints/best.pt` and restore `configs/base.yaml` (version, `infer.preprocess_mode: circular`, `threshold: 0.3931`, `tta_mode: hflip`, `data.manifest_path: manifest_preprocessed.csv`).
...
## 3. Current Configuration (v31_v8b_fusion_quickqual_v1 — Active Deployment Alias)

The system's active inference config points to **v31_v8b_fusion_quickqual_v1**. `artifacts/checkpoints/best.pt` currently contains the composite QuickQual-line v31 classifier + QuickQual-line v8b segmenter + numeric StandardScaler/LogReg meta-classifier checkpoint, so the fixed deployment alias and `configs/base.yaml` are aligned.

> **Metric boundary**: v31_v8b_fusion_quickqual_v1 uses the formal DDR external_test 20% calibration / 80% holdout policy. Holdout metrics are AUROC 0.9341 / threshold 0.06 / sensitivity 0.8234 / specificity 0.9086. The runtime compact metrics alias is `artifacts/evaluations/external_test_v31_v8b_fusion_quickqual_v1_best_metrics.json`; its source is `artifacts/runs/99_misc/v31_v8b_late_fusion_quickqual_v1/evaluations/v31_v8b_late_fusion_quickqual_v1_metrics.json`, key `classification_domains:late_fusion`. `artifacts/checkpoints/best.pt` metadata confirms `architecture=v31_v8b_fusion`, `optimal_threshold=0.06`, classifier source `artifacts/runs/99_misc/v31_no_se_gated_quickqual_v1/checkpoints/best.pt`, and segmenter source `artifacts/runs/99_misc/seg_evidence_v8b_quickqual_v1/checkpoints/best.pt`. Previous active rollback is `v31_v8b_fusion_features_hflip_v2`, backed up at `artifacts/checkpoints/best_pre_quickqual_v1_20260529.pt.bak`.
        ...
        → Messidor 학습 편입 + DDR 외부 테스트 교체 → artifacts/runs/02_domain_generalization/v7_messidor_train/checkpoints/best.pt  [DDR AUROC 0.8725]
          ...
          → 512px Messidor train backbone → artifacts/runs/03_resolution_layercam/v7_512_messidor_train/checkpoints/best.pt [DDR AUROC 0.9046]
            ...
            → true no-attention + Dice+BCE seg loss → artifacts/runs/07_lesion_evidence/v31_no_se_gated/checkpoints/best.pt [val AUROC 0.9993, DDR AUROC 0.9160, test XAI AUPRC 0.1409] (v31 base classifier)
              ...
              ↳ QuickQual-line v31 + v8b fusion deployment → artifacts/checkpoints/best.pt [DDR holdout AUROC 0.9341, threshold 0.06, QuickQual-line v8b lesion evidence] ← ACTIVE DEPLOYMENT ALIAS
...
- **Active deployment**: Active config points to `v31_v8b_fusion_quickqual_v1`. `artifacts/checkpoints/best.pt` contains the composite QuickQual-line v31 classifier + QuickQual-line v8b segmenter + numeric StandardScaler/LogReg meta-classifier checkpoint. Formal DDR calibration/holdout policy uses threshold 0.06; holdout AUROC 0.9341, Sens 0.8234, Spec 0.9086. The active runtime sets `infer.preprocess_mode: none` because backend QuickQual preprocessing is expected to happen before AI inference. The previous hflip/circular active alias is backed up at `artifacts/checkpoints/best_pre_quickqual_v1_20260529.pt.bak`; the older fusion alias remains at `artifacts/checkpoints/best_pre_features_hflip_v2_20260527.pt.bak`.
