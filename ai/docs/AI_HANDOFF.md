...
- **Current lesion evidence candidate**: `seg_evidence_v8b_ddrseg_tjdr_maplesfix` is the strongest standalone lesion segmentation evidence run after the MAPLES ROI fix. Best epoch 38, val mDice 0.3388. Test best metrics: IDRiD mDice 0.4151 / union IoU 0.3903, MAPLES mDice 0.2928 / union IoU 0.2121, TJDR mDice 0.3788 / union IoU 0.3149, DDR_SEG mDice 0.3945 / union IoU 0.2880. It is the Phase 4-G current best baseline, but not deployment-active. v31 remains the classifier deployment; backend/frontend integration is out of the current AI-side change scope.
- **Phase 4-G G-4 grounded classifier re-entry completed**: v8b lesion maps were reduced to scalar evidence features and fitted with calibrated logistic classifiers. The best variant was `v8b_evidence_classifier_grid_v1` (all train domains, C-grid), with DDR external_test AUROC 0.8942, threshold 0.56, Sens 0.7639, Spec 0.8674. This is below active v31 AUROC 0.9160, so v8b evidence features do not replace the deployment classifier. Phase 4-G closes with v8b as standalone lesion evidence baseline only; deployment remains v31.
...
- **v31 synchronized-transform rerun**: `v31_syncfix_rerun` reran the active v31 architecture after fixing classifier aux_seg image/mask transform sync. Dry-run confirmed `train_dataset_type=SegmentationManifestDataset` and `synchronized_mask_transform=True`; training completed with best epoch 9 / val AUROC 0.9993. DDR external_test regressed to AUROC 0.9082, optimal thr 0.24, Sens 0.7639, Spec 0.8905. IDRiD block4 Layer-CAM also regressed on AUPRC/AUC-IoU/IoU20 (0.1215 / 0.0394 / 0.0600 vs active v31 0.1409 / 0.0496 / 0.0785), while MAPLES remained weaker (AUPRC 0.0125, IoU20 0.0067). Follow-up method sweep found active v31 Grad-CAM is slightly higher than Layer-CAM on IDRiD AUC-IoU/IoU20 (0.0555 / 0.0827 vs 0.0496 / 0.0785), but syncfix still underperforms. Seed repeats did not rescue it: seed43 DDR AUROC 0.8999, seed44 DDR AUROC 0.9176 but IDRiD XAI IoU20 stayed <= 0.0661. Result: `NOT_PROMOTED`; active v31 remains deployment. Canonical summary: `.omc/research/v31_syncfix_rerun_result.json` plus `artifacts/runs/07_lesion_evidence/v31_syncfix_seed43|44/`.
