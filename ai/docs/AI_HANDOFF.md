...
- **Model**: Active deployment is `v31_v8b_fusion_v2`: v31_no_se_gated remains the base classifier score source, v8b provides lesion segmentation evidence, and a numeric StandardScaler+LogReg meta-classifier produces the final abnormal probability. Formal DDR 20% calibration / 80% holdout metrics: AUROC 0.9403, threshold 0.38, sensitivity 0.8118, specificity 0.9238. Phase 4-E shortcut audit still applies to the v31 base path, so product wording should describe v8b output as separately detected lesion candidate areas rather than causal proof.
...
## 3. Current Configuration (v31_v8b_fusion_v2 — Active Deployment Alias)

The system's active inference config points to **v31_v8b_fusion_v2**. `artifacts/checkpoints/best.pt` currently contains the composite v31 + v8b + numeric meta-classifier checkpoint, so the fixed deployment alias and `configs/base.yaml` are aligned.

> **Metric boundary**: v31_v8b_fusion_v2 uses the formal DDR external_test 20% calibration / 80% holdout policy. Holdout metrics are AUROC 0.9403 / threshold 0.38 / sensitivity 0.8118 / specificity 0.9238 (`artifacts/evaluations/external_test_v31_v8b_fusion_v2_best_metrics.json`). v31 remains preserved at `artifacts/runs/07_lesion_evidence/v31_no_se_gated/checkpoints/best.pt` and `artifacts/checkpoints/best_pre_fusion_v31_only.pt.bak`.
        ...
        → Messidor 학습 편입 + DDR 외부 테스트 교체 → artifacts/runs/02_domain_generalization/v7_messidor_train/checkpoints/best.pt  [DDR AUROC 0.8725]
          ...
          → 512px Messidor train backbone → artifacts/runs/03_resolution_layercam/v7_512_messidor_train/checkpoints/best.pt [DDR AUROC 0.9046]
            ...
            → true no-attention + Dice+BCE seg loss → artifacts/runs/07_lesion_evidence/v31_no_se_gated/checkpoints/best.pt [val AUROC 0.9993, DDR AUROC 0.9160, test XAI AUPRC 0.1409] (v31 base classifier)
              ...
              ↳ v31 + v8b score-level fusion deployment → artifacts/checkpoints/best.pt [DDR holdout AUROC 0.9403, threshold 0.38, v8b lesion evidence] ← ACTIVE DEPLOYMENT ALIAS
...
- **Active deployment**: Active config points to `v31_v8b_fusion_v2`. `artifacts/checkpoints/best.pt` contains the composite v31 classifier + v8b segmenter + numeric StandardScaler/LogReg meta-classifier checkpoint. Formal DDR calibration/holdout policy uses threshold 0.38; holdout AUROC 0.9403, Sens 0.8118, Spec 0.9238. The previous v31-only alias is backed up at `artifacts/checkpoints/best_pre_fusion_v31_only.pt.bak` and the source v31 checkpoint remains under `artifacts/runs/07_lesion_evidence/v31_no_se_gated/checkpoints/best.pt`.
...
- **Current lesion evidence path**: `seg_evidence_v8b_ddrseg_tjdr_maplesfix` is now the active lesion evidence module inside `v31_v8b_fusion_v2`. Best epoch 38, val mDice 0.3388. Test best metrics: IDRiD mDice 0.4151 / union IoU 0.3903, MAPLES mDice 0.2928 / union IoU 0.2121, TJDR mDice 0.3788 / union IoU 0.3149, DDR_SEG mDice 0.3945 / union IoU 0.2880.
...
- **Phase 4-G G-5 late fusion completed and deployed**: `v31_v8b_late_fusion_sweep_v1` combines v31 probability/logit with v8b scalar lesion-evidence features. The formal threshold policy uses a deterministic stratified DDR 20% calibration / 80% holdout split from `external_test`, then selects the highest-specificity threshold satisfying the v31 sensitivity guard. Calibration split: 2,504 images (normal 1,253 / abnormal 1,251). Holdout split: 10,018 images (normal 5,013 / abnormal 5,005). Selected threshold is **0.38**. Holdout metrics: AUROC 0.9403, Acc 0.8678, Sens 0.8118, Spec 0.9238, F1 0.8599. `v31_v8b_fusion_v2` packages this score-level fusion and v8b overlay into the active AI deployment path without backend/frontend code changes.
- **Threshold**: Active `infer.threshold` is 0.38, aligned with the v31+v8b fusion holdout threshold.
...
- **Phase 4-G G-4 completed**: `drscreen/cli/lesion_evidence_classifier.py` was added for diagnostic-only scalar classifiers over v8b lesion maps. Variants tested: all train domains AUROC 0.8828, classification-domain-only AUROC 0.8479, APTOS-only AUROC 0.8725, and C-grid AUROC 0.8942 on DDR external_test. Best G-4 still underperforms v31 AUROC 0.9160, so grounded classifier re-entry through scalar lesion evidence alone is not promoted. G-5 late fusion is the follow-up Sprint 5 diagnostic.
